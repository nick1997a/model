...
class Net(object):
    ...
    def modelvv(self,dropout,drug_layers,protein_strides,filters,fc_layers,prot_vec=False,prot_len=2500,activation='relu',
        ...
        ###################4444444444444444  step 4    define protein convolution
        ...
        model_final = Model(inputs=[input_d,input_d2,input_p,enc_inputs,dec_inputs],outputs=model_ttt)
    ...
    def validation(self,drug_feature,drug_feature2,protein_feature,protein_feature2,Label,n_epoch=10,batch_size =32,**kwargs):
        auc_temp = 0
        auclist = []
        auprlist = []
        for i in range(n_epoch):
            self.__model_t.fit([drug_feature,drug_feature2,protein_feature,protein_feature2,protein_feature2],Label,epochs=i+1,verbose=1,initial_epoch=i,batch_size=batch_size,shuffle=True)
            for dataset in kwargs:
                ...
                prediction = self.__model_t.predict([test_d,test_d2, test_p,test_p2,test_p2])
                #####################evaluate our model
                ...
                if (AUC > auc_temp):
                    ...
                    lab = finalsets['Label']
                    final_predition = self.__model_t.predict([drug_fea, drug_fea2, protein_fea, protein_fea2, protein_fea2])
